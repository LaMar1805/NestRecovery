...
function addCountries(url:string, a: string, b: string) {
	let tempUrl = url;
	if (a != null) {
		let tempUrlOne = new URL(tempUrl);
		tempUrl += ((tempUrlOne.search == "") ? "?" : "&") + "token_countries=" + a;
	}
	if (b != null) {
		let tempUrlTwo = new URL(tempUrl);
...
// @ts-ignore
export default function signUrl(url, securityKey, expirationTime = 3600, userIp, isDirectory = false, pathAllowed, countriesAllowed, countriesBlocked) {
	/*
